...
const HomePage = () => {
    ...
    const deleteWeather = (index) => {
        ...
        const filterData = weatherData.filter((list, i) => i !== index)        
        setWeatherData(filterData)  
        
        setHighLight((prev) => prev.filter(i => i !== index))
    }

    const getSearchData = () => {
        const matchedRow = weatherData.map((data, index) => (
            data.city.toLowerCase().includes(search.toLowerCase()) ? index : null)).filter(index => index !== null)
    ...
    return(
        <div className="container">
            <header className="header">
                Rushikesh Weather App
            </header>

            <div className="weather_container">
                <div className="weather_left">
                    <div onClick={getHightBtn} className="get_btn"    
                    >Get Weather</div>
                    <div className="data_list">
                        <div className="high_city">City</div>
                        {city.map((list, index) => (
                            <div className= {`list_city ${highlight.includes(index) ? "hilight" : ""}`} key={index}>{list}</div>
                        ))}
                    </div>
                </div>
                <div className="weather_right">
                    <div className="search">
                        <input type="text" className="input_search" placeholder="City Name" value={search} onChange={(e) => setSearch((e.target.value))}/>
                            ...
                            <tbody className="tbody">
                                {weatherData.length === 0 ? (
                                    ...
                                    <tr key={index} className={`tbody_tr ${highlightedRows.includes(index) ? "tbody_high" : ""}`}>
                                        ...
                                        <td className="tbody_td">{list.description}</td>
